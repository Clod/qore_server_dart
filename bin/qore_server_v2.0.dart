...
/*      loggerNoStack.i("Connecting to DataBase...");
  ...
  //      if (connectedToDB == true) {
          if (action == Commands.getPatientsByLastName.index) {
            conn = await connectToDB();
            responseMessage = await getPatientsByLastName(decoded, conn!);
            logger.d(responseMessage);
          } else if (action == Commands.getPatientsByIdDoc.index) {
            conn = await connectToDB();
            responseMessage = await getPatientsByIdDoc(decoded, conn);
            logger.d(responseMessage);
          } else if (action == Commands.addPatient.index) {
            conn = await connectToDB();
            String patient = utf8.decode(intList.sublist(3));
            responseMessage = await addPatient(patient, conn!);
          } else if (action == Commands.updatePatient.index) {
            String patient = utf8.decode(intList.sublist(3));
            responseMessage = await updatePatient(patient, conn);
          } else if (action == Commands.lockPatient.index) {
            String patient = utf8.decode(intList.sublist(3));
            conn = await connectToDB();
            responseMessage = await lockPatient(patient, conn!);
          } /*else {
        ...
        logger.i("Sending response back to server");
...
Future<String> lockPatient(String patientData, MySQLConnection conn) async {
...
Future<String> getPatientsByLastName(String s, MySQLConnection conn) async {
...
Future<String> getPatientsByIdDoc(String s, MySQLConnection? conn) async {
...
Future<String> addPatient(String patientData, MySQLConnection conn) async {
...
Future<String> updatePatient(String patientData, MySQLConnection? conn) async {
